...
class EasyPostClient:
    ...
    def __init__(self, api_key: Optional[str] = None, test_mode: Optional[bool] = None):
        ...
        Args:
            api_key: EasyPost API key (defaults to EASYPOST_API_KEY env var)
            test_mode: Use test mode (defaults to EASYPOST_TEST_MODE env var)
        """
        self.api_key = api_key or os.getenv("EASYPOST_API_KEY", "")
        self.test_mode = test_mode if test_mode is not None else os.getenv("EASYPOST_TEST_MODE", "true").lower() == "true"
    ...
    def get_rates(self, from_address: Address, to_address: Address,
        ...
        logger.info(f"Creating shipment to get rates{sig_info}")
        ...
        # Map signature confirmation to EasyPost delivery_confirmation option
        if signature_confirmation:
            ...
            shipment_data["options"] = {
                "delivery_confirmation": sig_map.get(signature_confirmation, "SIGNATURE")
            }
