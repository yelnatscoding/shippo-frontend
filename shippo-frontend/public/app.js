...
const ShippingApp = {
    ...
    parseAddress(text) {
        ...
        for (let i = 0; i < lines.length; i++) {
            // Check for pattern: "CA 93203 Arvin" or "CA 93203-1234 Arvin" (State ZIP City)
            ...
            // Check for Backmarket format: "20886 Gaithersburg MD" (ZIP City State)
            const zipCityStateMatch = lines[i].match(/^(\d{5}(?:-\d{4})?)\s+(.+?)\s+([A-Z]{2})$/i);
            if (zipCityStateMatch) {
                const state = zipCityStateMatch[3].toUpperCase();
                if (stateAbbrevs.includes(state)) {
                    parsed.zip = zipCityStateMatch[1];
                    parsed.city = zipCityStateMatch[2].trim();
                    parsed.state = state;
                    stateZipCityFound = true;
                    lines[i] = ''; // Clear this line
                    break;
            ...
            // Check for standard US format: "Austin, TX 78701" or "Austin TX 78701" (City, State ZIP)
            const cityStateZipMatch = lines[i].match(/^(.+?)[,\s]+([A-Z]{2})[,\s]+(\d{5}(?:-\d{4})?)$/i);
            if (cityStateZipMatch) {
                const state = cityStateZipMatch[2].toUpperCase();
                if (stateAbbrevs.includes(state)) {
                    parsed.city = cityStateZipMatch[1].replace(/,\s*$/, '').trim();
                    parsed.state = state;
    ...
    parseInlineAddress(text) {
        ...
        // Try to match standard US format first: "Name, Street, City, State ZIP"
        // Example: "John Doe, 123 Main St, Austin, TX 78701"
        const usFormatMatch = text.match(/^(.+?),\s*(.+?),\s*(.+?),\s*([A-Z]{2})\s+(\d{5}(?:-\d{4})?)$/i);
        if (usFormatMatch && stateAbbrevs.includes(usFormatMatch[4].toUpperCase())) {
            parsed.name = usFormatMatch[1].trim();
            parsed.street = usFormatMatch[2].trim();
            parsed.city = usFormatMatch[3].trim();
            parsed.state = usFormatMatch[4].toUpperCase();
            parsed.zip = usFormatMatch[5];
            return parsed;
        }

        // Try: "Street, City, State ZIP" (no name)
        const usNoNameMatch = text.match(/^(.+?),\s*(.+?),\s*([A-Z]{2})\s+(\d{5}(?:-\d{4})?)$/i);
        if (usNoNameMatch && stateAbbrevs.includes(usNoNameMatch[3].toUpperCase())) {
            parsed.street = usNoNameMatch[1].trim();
            parsed.city = usNoNameMatch[2].trim();
            parsed.state = usNoNameMatch[3].toUpperCase();
            parsed.zip = usNoNameMatch[4];
            return parsed;
        }
    ...
    async compareRates() {
        try {
            ...
            // Show loading - validate first
            this.showLoading('Validating address...');

            // Validate address first
            let validatedAddress = toAddress;
            try {
                const validateResponse = await axios.post(`${this.config.apiUrl}/validate`, {
                    address: toAddress
                });

                if (validateResponse.data.success && validateResponse.data.data.validated_address) {
                    validatedAddress = validateResponse.data.data.validated_address;
                    this.state.validatedToAddress = validatedAddress;
                    this.state.validationResult = validateResponse.data.data;
                }
            } catch (valError) {
                console.warn('Address validation failed, using original address:', valError);
            }

            // Show loading - get rates
            this.showLoading('Comparing rates from all providers...');

            // Call API with validated address
            ...
            if (response.data.success) {
                ...
                // Render validated address + rates
                this.renderRates(response.data.data, response.data.errors, validatedAddress, toAddress);
            } else {
    ...
    renderRates(ratesData, errors, validatedAddress = null, originalAddress = null) {
        ...
        // Show validated/confirmed shipping address at the top
        if (validatedAddress && validatedAddress.street1) {
            const vStreet = validatedAddress.street1 || '';
            const vCity = validatedAddress.city || '';
            const vState = validatedAddress.state || '';
            const vZip = validatedAddress.zip || '';
            const vName = validatedAddress.name || originalAddress?.name || '';
            const vStreet2 = validatedAddress.street2 || '';

            const oStreet = originalAddress?.street1 || '';
            const oCity = originalAddress?.city || '';
            const oState = originalAddress?.state || '';
            const oZip = originalAddress?.zip || '';

            const addressChanged = originalAddress && (
                vStreet.toLowerCase() !== oStreet.toLowerCase() ||
                vCity.toLowerCase() !== oCity.toLowerCase() ||
                vState.toLowerCase() !== oState.toLowerCase() ||
                vZip !== oZip
            );

            html += `
                <div class="card mb-3 ${addressChanged ? 'border-warning' : 'border-success'}">
                    <div class="card-header ${addressChanged ? 'bg-warning text-dark' : 'bg-success text-white'}">
                        <i class="bi ${addressChanged ? 'bi-exclamation-triangle' : 'bi-check-circle'}"></i>
                        ${addressChanged ? 'Address Corrected' : 'Shipping Address Confirmed'}
                    </div>
                    <div class="card-body">
                        <div class="row">
                            <div class="col-md-6">
    ...
    async purchaseLabel(rateId, provider, signature = false) {
        try {
            ...
            this.showLoading('Purchasing label...');

            // Use validated address if available, otherwise use original
            const toAddress = this.state.validatedToAddress || this.state.toAddress;
            ...
            if (response.data.success) {
                ...
                if (labelData.label_url_temp) {
                    window.open(labelData.label_url_temp, '_blank');
                }
    ...
    renderHistory(history) {
        ...
        history.forEach(label => {
            ...
            html += `
                        <button class="btn btn-sm btn-outline-success me-1" onclick='ShippingApp.copyShipment(${JSON.stringify(label).replace(/'/g, "\\'")})' title="Ship Again">
                            <i class="bi bi-arrow-repeat"></i>
                        </button>
            `;

            // Add void button for ShipEngine labels
            if (label.provider === 'shipengine' && label.label_id && !label.voided) {
                html += `
                        <button class="btn btn-sm btn-outline-danger" onclick='ShippingApp.confirmVoidLabel("${label.label_id}", "${label.tracking_number}", "${label.provider}")' title="Void Label">
    ...
    hideLoading() {
        ...
        // Also manually remove backdrop if it persists
    ...
    // Confirm void label with user
    confirmVoidLabel(labelId, trackingNumber, provider) {
        const confirmed = confirm(
            `Are you sure you want to void this label?\n\n` +
            `Tracking: ${trackingNumber}\n` +
            `Provider: ${provider}\n\n` +
            `This action cannot be undone. A refund will be requested from the carrier.`
        );

        if (confirmed) {
            this.voidLabel(labelId, trackingNumber, provider);
        }
    },

    // Void a shipping label
    async voidLabel(labelId, trackingNumber, provider) {
        try {
            this.showLoading('Voiding label...');

            const response = await axios.post(`${this.config.apiUrl}/void`, {
                label_id: labelId,
            ...
            if (response.data.success) {
                ...
                // Refresh history to show voided status
                this.loadHistory();
            } else {
                this.showError(response.data.error || 'Failed to void label');
            }

        } catch (error) {
            this.hideLoading();
            console.error('Error voiding label:', error);
            this.showError(error.response?.data?.error || error.message || 'Failed to void label');
        }
    }
};
