...
class handler(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        try:
            ...
            signature_confirmation = 'STANDARD' if signature else None
            ...
            if provider == 'shippo':
                label = self._purchase_shippo_label(rate_id, label_format, signature_confirmation)
            elif provider == 'easypost':
                label = self._purchase_easypost_label(rate_id, label_format, signature_confirmation)
            ...
            if GoogleDriveUploader and os.environ.get('GOOGLE_SERVICE_ACCOUNT_JSON'):
                try:
            ...
            response_data = {
                ...
                'data': {
                    ...
                    'created_at': label.created_at.isoformat() if hasattr(label.created_at, 'isoformat') else (label.created_at or datetime.now().isoformat()),
                    ...
                    'label_id': getattr(label, 'label_id', None)  # For voiding
                }
            }
    ...
    def _purchase_shippo_label(self, rate_id, label_format, signature_confirmation=None):
    ...
    def _purchase_easypost_label(self, rate_id, label_format, signature_confirmation=None):
    ...
    def _purchase_shipengine_label(self, rate_id, label_format, signature_confirmation=None):
    ...
    def _purchase_easyship_label(self, rate_id, label_format, signature_confirmation=None):
        """Purchase label from Easyship"""
        client = EasyshipClient(
            api_key=os.environ['EASYSHIP_API_KEY']
        )
        # Easyship doesn't have a purchase_label method yet, so we just pass through
        # If it exists, call it with signature_confirmation
